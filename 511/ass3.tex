...
\else
\missingfigure{Schedule graphic only appears in PDF mode.}
\fi
...
\todo[inline]{The cost of corrective measures???}
...
\todo[inline]{How will you ensure that your documents and code have been independently reviewed by
  ...
  How assured that fulfilling commitments to rocesses spec'd in requirements spec, spmp, supporting
  plans, standards, procedures, guidelines? Analysis, inspections, reviews, audits, assessments.
}
...
\todo[inline]{Describe how you will manage the key software documents produced by the project, in
  ...
  What documents generated?  Templates/standards?  Who responsible for providing info, generating
  docs, reviewing, accepting?  Which are placed in ver. control?  When are review/baseline versions
  due?

  Nondeliverables: requirements specs, design documentation, traceability matrices, test plans,
  meeting minutes, review reports, action items, change requests, defect reports.

  Deliverables: source code, binary code, manual, on-line help, regression test suite, configuration
  library, principles of operation, maintenance guide, stuff from \ref{deliverables}.
}
...
\end{tabular}
% \begin{description}
% \item[SPMP] --~ Uses the JDI SPMP template.  SPM provides content, review and acceptance by CHG and
%   JDI management.
% \item[SRS] --~ Uses the JDI SRS template.  RM responsible for content, review and acceptance by CHG
%   management.
% \end{description}
...
\todo[inline]{Describe the work that will be contracted out, if any, deliverables, schedules for
  ...
  How selected and managed? Who resp. for preparing mgmt plans? Who resp. for providing technical/
